...
// debut route ajax Grid post presse
...
function custom_api_get_posts_callback($request){
    ...
    if($query->have_posts()) {
        ...
        while($query->have_posts()) {
            ...
            $post_array['posts'][$i]['url'] = get_field('lien_vers_article_presse');
            $post_array['posts'][$i]['title'] = get_the_title();
            $post_array['posts'][$i]['propos'] = get_the_date('F Y');
...
function requete_evenements($request){
    $pourMois = $request-> get_param('periode');
    if($pourMois=="passe"){
        $aujourdhui = date('Y-m-d', strtotime('-1 year'));
        $dateFin = date('Y-m-d');
            $args = array(
    ...
    $post_array= [];
    $post_array['date'] = traduireMois(date('F'))." ".date('Y');
    if($query->have_posts()) {
...
add_action( 'init', 'traduireMois', 1 );
?>